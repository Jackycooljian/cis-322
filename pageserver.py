...
def respond(sock):
    ...
    if len(parts) > 1 and parts[0] == "GET":
        ...
        expandedname = os.path.splitext(parts[1])[1]
        
        if expandedname !=".html" and expandedname !=".css":
            transmit(STATUS_FORBIDDEN ,sock)
            transmit("403.1 error, please write ture expande name", sock)
        
        else:
            if "//" in filename or ".." in filename or "~" in filename:
                transmit(STATUS_FORBIDDEN, sock)
                transmit("403 error, please check the name, it can't have '//','..','~'", sock) 
            else:
                if os.path.isfile("pages"+file):
                    file = open("pages"+file)
                    transmit(STATUS_OK, sock)
                    transmit(file.read(), sock)
                else:
                    transmit(STATUS_NOT_FOUND, sock)
                    transmit("404 error,  "+ os.path.basename(parts[1]) + " was not found!!", sock)              
    else:
